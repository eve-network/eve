...
// GetDenomExchangeRate gets the consensus exchange rate of denominated in the denom asset from the store.
func (k Keeper) GetDenomExchangeRate(ctx sdk.Context, denom string) (sdk.Dec, error) {
...
// SetDenomExchangeRate sets the consensus exchange rate of  USD in the denom asset to the store.
func (k Keeper) SetDenomExchangeRate(ctx sdk.Context, denom string, exchangeRate sdk.Dec) {
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshal(&sdk.DecProto{Dec: exchangeRate})
	store.Set(types.GetExchangeRateKey(denom), bz)
}

// SetDenomExchangeRateWithEvent sets the consensus exchange rate of USD
// denominated in the denom asset to the store with ABCI event
func (k Keeper) SetDenomExchangeRateWithEvent(ctx sdk.Context, denom string, exchangeRate sdk.Dec) {
	k.SetDenomExchangeRate(ctx, denom, exchangeRate)
...
// DeleteExchangeRate deletes the consensus exchange rate of denominated in the denom asset from the store.
func (k Keeper) DeleteExchangeRate(ctx sdk.Context, denom string) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(types.GetExchangeRateKey(denom))
}

// IterateExchangeRates iterates over rates by denom in the store
