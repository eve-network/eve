...
func TestOrganizeAggregate(t *testing.T) {
	...
	sdrBallot := types.ExchangeRateBallot{
		types.NewVoteForTally(sdk.NewDec(17), types.TestDenomB, ValAddrs[0], power),
		types.NewVoteForTally(sdk.NewDec(10), types.TestDenomB, ValAddrs[1], power),
		types.NewVoteForTally(sdk.NewDec(6), types.TestDenomB, ValAddrs[2], power),
	}
	...
	sort.Sort(sdrBallot)
	sort.Sort(krwBallot)
	sort.Sort(ballotMap[types.TestDenomB])
	sort.Sort(ballotMap[types.TestDenomC])

	require.Equal(t, sdrBallot, ballotMap[types.TestDenomB])
