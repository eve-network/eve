...
func terrac() ([]banktypes.Balance, []config.Reward, int, error) {
	...
	rpc := config.GetTerracConfig().API + "/cosmos/staking/v1beta1/validators?pagination.limit=" + strconv.Itoa(utils.LimitPerPage) + "&pagination.count_total=true"
	...
	for validatorIndex, validator := range validators {
		url := config.GetTerracConfig().API + "/cosmos/staking/v1beta1/validators/" + validator.OperatorAddress + "/delegations?pagination.limit=" + strconv.Itoa(utils.LimitPerPage) + "&pagination.count_total=true"
	...
	fetchTokenPrice := fetchTokenPriceWithRetry(fetchTerracTokenPrice)
