...
func NewManager(isUpgrade bool, isFork bool, isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("eve-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) RunHermesResource(chainAID, eveARelayerNodeName, eveAValMnemonic, chainBID, eveBRelayerNodeName, eveBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("EVE_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("EVE_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("EVE_A_E2E_VAL_MNEMONIC=%s", eveAValMnemonic),
				fmt.Sprintf("EVE_B_E2E_VAL_MNEMONIC=%s", eveBValMnemonic),
				fmt.Sprintf("EVE_A_E2E_VAL_HOST=%s", eveARelayerNodeName),
				fmt.Sprintf("EVE_B_E2E_VAL_HOST=%s", eveBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
