...
depending on the Eve codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
Eve version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older Eve version to the container,
...
1. Chain Initialization
    ...
    of the previous Eve version, inside `configurer/upgrade.go`. This is
    ...
    current branch version of Eve codebase
    ...
    with IBC initialized at the previous Eve version
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older Eve version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch Eve version is spun up to continue with testing
...
such as the `app.toml`. This package directly depends on the Eve
codebase.
...
production Eve container to start the `eved` service.
...
- `EVE_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If EVE_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `EVE_E2E_SKIP_IBC` - when true, skips the IBC tests tests.

- `EVE_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `EVE_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `EVE_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `EVE_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `EVE_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
...
```json
{
    ...
    "env": {
        ...
        "EVE_E2E_UPGRADE_VERSION": "v10",
        "EVE_E2E_DEBUG_LOG": "true",
        "EVE_E2E_FORK_HEIGHT": "4713065" # this is v10 fork height.
    }
}
```
