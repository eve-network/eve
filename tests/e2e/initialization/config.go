...
var (
	...
	InitBalanceStrA = fmt.Sprintf("%d%s,%d%s,%d%s", EveBalanceA, BaseDenom, StakeBalanceA, StakeDenom, IonBalanceA, IonDenom)
	InitBalanceStrB = fmt.Sprintf("%d%s,%d%s,%d%s", EveBalanceB, BaseDenom, StakeBalanceB, StakeDenom, IonBalanceB, IonDenom)
	EveToken        = sdk.NewInt64Coin(BaseDenom, IbcSendAmount)  // 3,300ueve
