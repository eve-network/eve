...
func setup(withGenesis bool, invCheckPeriod uint) (*EveApp, GenesisState) {
	db := dbm.NewMemDB()
	encCdc := MakeTestEncodingConfig()
	app := NewEveApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, invCheckPeriod, encCdc, GetEnabledProposals(), EmptyAppOptions{}, nil)
	if withGenesis {
...
func NewSimappWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *EveApp {
	...
	// app := NewEveApp(options.Logger, options.DB, nil, true, options.SkipUpgradeHeights, options.HomePath, options.InvCheckPeriod, options.EncConfig, options.AppOpts, nil)
	app := NewEveApp(options.Logger, options.DB, nil, true, options.SkipUpgradeHeights, options.HomePath, options.InvCheckPeriod, options.EncConfig, GetEnabledProposals(), options.AppOpts, nil)
...
func genesisStateWithValSet(t *testing.T,
	app *EveApp, genesisState GenesisState,
	...
	validators := make([]stakingliqidtypes.Validator, 0, len(valSet.Validators))
	delegations := make([]stakingliqidtypes.Delegation, 0, len(valSet.Validators))
	...
	for _, val := range valSet.Validators {
		...
		require.NoError(t, err)
		validator := stakingliqidtypes.Validator{
			...
			Description:     stakingliqidtypes.Description{},
			UnbondingHeight: int64(0),
			UnbondingTime:   time.Unix(0, 0).UTC(),
			Commission:      stakingliqidtypes.NewCommission(sdk.ZeroDec(), sdk.ZeroDec(), sdk.ZeroDec()),
		}
		validators = append(validators, validator)
		delegations = append(delegations, stakingliqidtypes.NewDelegation(genAccs[0].GetAddress(), val.Address.Bytes(), sdk.OneDec(), false))

	}
	// set validators and delegations
	stakingGenesis := stakingliqidtypes.NewGenesisState(stakingliqidtypes.DefaultParams(), validators, delegations)
	genesisState[stakingliqidtypes.ModuleName] = app.AppCodec().MustMarshalJSON(stakingGenesis)
	...
	balances = append(balances, banktypes.Balance{
		Address: authtypes.NewModuleAddress(stakingliqidtypes.BondedPoolName).String(),
...
// SetupWithGenesisAccounts initializes a new EveApp with the provided genesis
...
func addTestAddrs(app *EveApp, ctx sdk.Context, accNum int, accAmt math.Int, strategy GenerateAccountStrategy) []sdk.AccAddress {
